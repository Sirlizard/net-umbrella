        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-1">
              ...
              <div className="space-y-2">
                {journals.map(j => (
                  <button key={j.id} onClick={() => setSelectedJournalId(j.id)} className={`w-full text-left px-3 py-2 rounded-lg border ${selectedJournalId === j.id ? 'border-[#28428c] bg-[#ffacd6]/10' : 'border-gray-200 hover:border-[#ffacd6]/50'}`}>
                    ...
                    <div className="text-xs text-[#624a4a]">Updated {new Date(j.updated_at).toLocaleString()}</div>
                  </button>
                ))}
              </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
              ...
              <div className="space-y-2 max-h-80 overflow-auto pr-1">
                {friends.map(f => (
                  <label key={f.id} className="flex items-center space-x-2 text-sm text-[#624a4a]">
          ...
          <div className="md:col-span-2">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
              ...
              {entries.length === 0 ? (
                <p className="text-sm text-[#624a4a]">No entries yet. Your reflections will appear here.</p>
              ) : (
                <div className="space-y-4">
                  {entries.map(e => (
                    <div key={e.id} className="border border-gray-100 rounded-lg p-4">
                      <div className="text-xs text-[#624a4a] mb-2">{new Date(e.created_at).toLocaleString()}</div>
